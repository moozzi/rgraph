...
module Codegen
  ...
  class Generator
    ...
    EXTRA_ENUM_MEMBERS = {
      GreedyColoringHeuristics: [%w[NEIGHBORS COLORED_NEIGHBORS]],
      LayoutGrid: [%w[NO_GRID NOGRID], %w[AUTO_GRID AUTOGRID]],
      Loops: [["IGNORE", 0]],
    }
    ...
    def self.process_enum(output_file, spec)
      ...
      return nil if IGNORED_ENUMS.include?(orig_name)
      return nil if !name.start_with?("igraph_") || name.start_with?("igraph_i_")
      ...
      return name unless remapped_name
      ...
      entries.each do |entry|
        ...
        begin
          ...
          raise StandardError, "enum key is not a valid Ruby identifier: #{key}" unless key == 1
    ...
    def self.process_file(output_file, input_file)
      ...
      input_file.each_line do |line|
        ...
        next unless !current_enum.empty? && !in_enum
    ...
    def self.longest_common_prefix_length(items)
      ...
      1.upto(min_length) do |i|
        ...
        break if prefixes.uniq.size > 1
        next if prefixes[0][-1] != "_"
